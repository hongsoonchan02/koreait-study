...
public class BoardFileServiceImpl implements BoardFileService {
	...
	public void saveFiles(Board board, List<MultipartFile> files) {
		...
		// 3. 파일을 로컬에 저장
		SaveFile saved = saveFilesToDisk(file, dir);
	...
	/**
		* 업로드 파일을 FILE_PATH_PREFIX 디렉토리에 저장하고 저장 결과를 반환하는 메서드
		* @param file 저장할 파일 정보
		* @param dir 저장할 디렉토리
		* @return SaveFile 저장 결과가 담긴 객체
		*/
	private SaveFile saveFilesToDisk(MultipartFile file, File dir) {
	...
	public boolean hasNewFiles(List<MultipartFile> files) {
		// 1. files 리스트가 없거나 비어있으면 false 반환
		if(files == null || files.isEmpty()) {
			return false;
		}
		
		// 2. 실제 업로드한 파일이 1개라도 존재한다면 true 반환
		for(MultipartFile file : files) {
